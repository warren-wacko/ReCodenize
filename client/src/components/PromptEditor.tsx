...
type PromptEditorProps = {
  ...
  submitting: boolean;
  submitLabel: { idle: string; publish?: string; saving: string };
  onSubmit: (values: PromptValues) => Promise<void>;
  /** Slot rendered between Cancel and Save (e.g. a Delete button on edit). */
...
export function PromptEditor({
  ...
  const [tplId, setTplId] = useState(templates?.length ? "blank" : "");
  const [title, setTitle] = useState(initialValues?.title ?? "");
  const [body, setBody] = useState(initialValues?.body ?? "");
  const [desc, setDesc] = useState(initialValues?.description ?? "");
  const [tool, setTool] = useState<Tool>(initialValues?.tool ?? "cursor");
  const [task, setTask] = useState<TaskType>(initialValues?.taskType ?? "rules");
  const [stack, setStack] = useState<string[]>(initialValues?.stack ?? []);
  ...
  return (
    <div className="np-shell">
      ...
      <div className="np-title-row">
        ...
        <h1 className="np-h1">{headline}</h1>
      </div>
        ...
        <div className="np-form">
          ...
          <div className="np-field">
            <label className="np-label" htmlFor="np-body">
              ...
              >
                {charCount.toLocaleString()} / {BODY_MAX.toLocaleString()} chars
              </span>
            </label>
            ...
            {isLong && (
              <span className="np-error">
                Prompt body is too long. Trim it under{" "}
                {BODY_MAX.toLocaleString()} characters.
              </span>
            )}
          </div>
